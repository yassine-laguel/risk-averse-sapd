...
# Load the data
data_test = pd.read_csv('data/arcene_valid.csv', header=None)
A_test = data_test.iloc[:, :-1].values
b_test = data_test.iloc[:, -1].values

# Remove rows with NaN values
A_test = A_test[~np.isnan(A_test).any(axis=1)]
b_test = b_test[~np.isnan(b_test)]

# Load the training data
data_train = pd.read_csv('data/arcene_train.csv', header=None)
A = data_train.iloc[:, :-1].values
b = data_train.iloc[:, -1].values

# Remove rows with NaN values
nan_indices = np.isnan(A).any(axis=1) | np.isnan(b)
A = A[~nan_indices]
b = b[~nan_indices]
...
for i in range(d):
    if A_test[:, i].max() == A_test[:, i].min():
        A_test[:, i] = np.zeros(n1)
    else:
        A_test[:, i] = (A_test[:, i] - A_test[:, i].min()) / (A_test[:, i].max() - A_test[:, i].min())
...
def generate_param_dct(rho, max_iter=1000, batch_size_x=10, batch_size_y=10):
    # Set running time, batch size, and simulation num
    para = {}
    ...
    # Set regularizer and strongly convex parameter
    para['lambda'] = 0.02
    para['mu_x'] = 0.02
    para['mu_y'] = 10
    ...
    # Find Lipschitz constant and other parameters
    para['A'] = A
    para['b'] = b
    ...
    para['x'] = np.zeros((d, para['maxIter']+1))
    para['y'] = np.zeros((n, para['maxIter']+1))
    para['x0'] = 2 * np.ones(d)
...
def article_plot(array1, array2, array3, array4, array5, array6, bins=30,
                title1=None, title2=None, title3=None, xlabel1=None,
                xlabel2=None, xlabel3=None, ylabel=None, title=None,
                labels=None):
...
#         legend = fig.legend(lines, labels, loc='lower center', ncol=len(labels), bbox_to_anchor=(0.5, -0.15), fontsize=12)
    ...
    if not os.path.exists('figs'):
        os.makedirs('figs')
    fig.savefig('figs/arcene_perfs.pdf', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    with open('data/arcene_opt_solution.json', 'w') as f:
        json.dump(data, f)

    # Compute Histograms
    with open('data/arcene_opt_solution.json', 'r') as f:
        data = json.load(f)
    ...
    para1 = generate_param_dct(rho1, max_iter=max_iter, batch_size_x=10, batch_size_y=10)
    para2 = generate_param_dct(rho2, max_iter=max_iter, batch_size_x=10, batch_size_y=10)
    ...
    for seed in range(n_samples):
        ...
        # write optimal solution to JSON file
    with open('data/arcene_performances.json', 'w') as f:
        json.dump(performances_dct, f)

    # PLOTTING
    title1 = r'Arcene - convergence'
    title2 = r'Arcene - intermediate histogram'
    title3 = r'Arcene - final histogram'

    with open('data/arcene_performances.json', 'r') as f:
    ...
    article_plot(arr1, arr2, arr3, arr4, arr5, arr6, bins=50,
                 ...
                 xlabel3=r"$\Vert y_k - x^\star \Vert^2 + \Vert y - y^\star\Vert^2$, $k=800$",
                 ylabel=r'$\Vert x_k - x^\star \Vert^2+ \Vert y_k - y^\star\Vert^2$', title=None,
                 labels=[r'$\rho_1=.989$', r'$\rho_2=.992$'])
